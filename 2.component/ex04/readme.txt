...
2) 제어 컴포넌트
    1). input, textarea, option과 같은 폼(<form/>) 컴포넌트 에서 사용자 입력에 따라
        state값이 변경되고 렌더링하는 컴포넌트를 제어컴포넌트

    2). 제어 컴포넌트 : 예제 src/02
        - '컴포넌트의 UI를 외부에서 변경할 수 없고 내부의 상태 변경으로 가능하다'라는
           리액트 컴포넌트의 작성 원칙을 준수 할 수 있다.
        - 사용자 입력 값을 제어(제한, validation)를 할 수 있다.
    3). 비제어 컴포넌트 : 예제 src/03 
        - 폼 컴포넌트를 반드시 제어 컨포넌트로 작성해야 하는 것은 아니다.
        - 상태를 제어하지 않는 비제어 컴포넌트를 만들 수 있다.

3) 상태 컴포넌트 vs 순수(Pure, Dumb) 컴포넌트 예제 : emaillist 
    1. 상태 컴포넌트
        - 상태를 관리하는 컴포넌트
    ...
    2. 순수 컴포넌트
        - 상태없이 속성(porps)로 화면만 렌더링하는 컴포넌트
        - 재사용성이 좋다. 테스트하기도 좋다.
    
    *3. 애플리케이션의 컴포넌트들은 상태 컴포넌트와 순수 컴포넌트로 분리하여 개발하는 것이 좋다. 예제) emaillist

    4. 어떤 컴포넌트가 상태 컴포넌트인가? 예제)emaillist
